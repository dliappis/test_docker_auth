...
func (db *redisTokenDB) GetValue(user string) (*TokenDBValue, error) {
	...
	if err == redis.Nil {
		glog.Errorf("Key <%s> doesn't exist\n", key)
		return nil, nil
	} else if err != nil {
		glog.Errorf("Error getting key <%s>: %s\n", key, err)
		return nil, fmt.Errorf("Error getting key <%s>: %s\n", key, err)
	}
...
func (db *redisTokenDB) StoreToken(user string, v *TokenDBValue, updatePassword bool) (dp string, err error) {
	...
	if db.encrypt_key != "" {
		...
		if err != nil {
			...
			return "", fmt.Errorf("Error encrypting key <%s>: %s\n", key, err)
		}
	}

	err = db.client.Set(key, data, 0).Err()
	if err != nil {
		glog.Errorf("Failed to store token data for user <%s>: %s", user, err)
		return "", fmt.Errorf("Failed to store token data for user <%s>: %s", user, err)
	}

	glog.V(2).Infof("Server tokens for <%s>: %x", user, string(data))
	return
}
...
func (db *redisTokenDB) DeleteToken(user string) error {
	glog.V(1).Infof("Deleting token for user <%s>", user)
